...
def env_factory(path, verbose=False, **kwargs):
    ...
    if 'ant' in path.lower():
      ...
      return partial(AntProgramEnv, env=env, models=ANT_LOW_TORQUE_MODELS, goal_threshold=0.5, \
            goal_detection_threshold=4.0, **config_dict)

    if 'minecraft' in path.lower():
      from abs_minecraft.gym_env import MinecraftEnv
      return partial(MinecraftEnv)
