...
# node in search tree
class SearchNode:
    def __init__(self, parent_node, current_sketch, cost, isleaf=False, score_C=5., cost_ratio=.5, lib_actions=None, allow_action_first=False):
        # store
    ...
    def get_score(self):
        ...
        if self.parent is None:
            return np.mean(self.score_list) - self.cost_ratio * self.cost
        return np.mean(self.score_list) - self.cost_ratio * self.cost + self.score_C * np.sqrt(np.log(self.parent.visit_num) / self.visit_num)
        ...
        if not self.isleaf:
            ...
            if self.parent is None:
                ...
                for lib_action in self.lib_actions:
                    ...
                    tmp_children.append(SearchNode(self, lib_action, 1, True))
                if self.allow_action_first:
                    # add pure C
                    p = Program().expand()[1]
                    p.stmts.pop(1)
                    p.stmts.pop(1)
                    p.stmts[0] = C()
                    tmp_children.append(SearchNode(self, p, 0, True))
...
class SearchTree:
    ...
    def display_all(self):
        ...
        while len(queue) != 0:
            ...
            mean_score = np.mean(cur_node.score_list) if len(cur_node.score_list) > 0 else -np.inf
            node_dict[cur_level].append((str(cur_node), cur_node.cost, cur_node.get_score(), cur_node.visit_num, mean_score))
            # add into queue
