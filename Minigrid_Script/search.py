...
class Node:
    ...
    def execute_and_update(self, program):
        for e in self.eval_seeds:
            ...
            if e in list(range(1, 25)):
                minigrid_implement.dsl.DSL_DEBUG = False
                print("stop to generate images")
            print(f"seed {e} completed ({done})")
        print(program.to_string_verbose())
        return program
    ...
    def add_queue(self, candidate, reward, robot, cost=None, is_return=False):
        ...
        if reward == 1:
            ...
            all_rewards = []
            for e in self.more_seeds:
                rst = execute_for_reward(self.get_robot(e), candidate, True)
                all_rewards.append(rst[1])
            # results = [
            ...
            if reward == 1 and candidate.find_actions()[0] is None:
                ...
                if success_rate == 1.0:
                    print("found a correct program", candidate)
                    pt = self.post_condition(candidate)
                    print(f"post condition is {pt}")
                    if self.found_one and pt:
    ...
    def test_program(self, prog):
        ...
        #         minigrid_implement.dsl.print_interaction()
        #         exit()
        # if self.task == "MiniGrid-MultiRoom-N6-v0":
            # if (
                # str(prog)
                # != " WHILE(front_is_clear) { IF(front_is_closed_door) { turn_left}  move} ; turn_right WHILE(not (goal_on_right)) { IF(left_is_clear) { turn_left}  IF(goal_present) { return}  IF(front_is_closed_door) { toggle}  IF(not (front_is_clear)) { turn_right}  move} ; turn_right WHILE(not (goal_present)) { move} ;; END (False)"
            # ):
    ...
    def test_for_success_rate(self, prog):
        ...
        for e in lst:
            ...
            if force_eval_robot.env.env.get_reward() == 1:
                ...
                print(f"seed {e} fail")
                failed.append(e)
            print(f"steps is {force_eval_robot.steps}")
    ...
    def eval_program(self, seed, robot, candidate, check_multiple):
        ...
        # no fuel
            ...
            # log and print
            log_and_print("\n no fuel with reward {} for \n {}".format(r, candidate))
            log_and_print(f"\n robot step is {eval_robot.steps}")
            self.candidates["no_fuel"].append((r, candidate))
            return self.FAIL_TYPE, eval_robot
        # complete
        elif candidate.complete():
    ...
    def search(self):
        ...
        while True:
            ...
            for iter in range(self.max_search_iter):
                if iter == 0:
                    old_counter = copy.deepcopy(minigrid_implement.dsl.search_counter)
                    old_counter = 0 if old_counter is None else old_counter
                if minigrid_implement.dsl.search_counter is not None:
                    if minigrid_implement.dsl.search_counter - old_counter > self.step_limit_per_sketch:
                        print("step limit exceed, breaking")
                        break
                log_and_print("[ITER] {}".format(iter))

                # get one program
                if iter == 1729: # or iter == 424:
                    # pdb.set_trace()
                    axxx = 1
                ...
                if eval_result == self.SUCCESS_TYPE:
                    ...
                    if not set_fail:
                        ...
                        if eval_result == self.MORE_WORK_TYPE:
                            ...
                            pt = self.post_condition(p)
                            print(f"post condition is {pt}")
                            if self.found_one and test_result == self.SUCCESS_TYPE and pt:
                                # break
                                raise SUCC(p)
                            else:
                                continue
                ...
                if iter == 0 and cond_type is None:
                    ...
                    if not self.allow_action_first:
                        break
...
if __name__ == "__main__":
    ...
    with open("library_ckpt_4", "rb") as library_f:
        library = pickle.load(library_f)
    with open("library_dict_ckpt_4", "rb") as library_dict_f:
    ...
    # more_seeds = [1]
    # eval_seeds = [i for i in range(0, 1000)]    
    eval_seeds =[1, 3, 4, 6, 11, 12, 14, 18, 20, 22, 25, 27, 36, ]
    ...
    debug_prog_str = "WHILE(front_is_clear) { IF(front_is_closed_door) { turn_left}  move}  turn_right WHILE(not (goal_present)) { WHILE(not (goal_on_right)) { IF(left_is_clear) { turn_left}  IF(goal_on_right) { turn_right}  IF(goal_present) { return}  IF(front_is_key) { turn_right}  IF(not (front_is_clear)) { IF(front_is_locked_door) { get_key pickup get_locked_door toggle} ELSE { turn_right}}  IF(front_is_closed_door) { toggle}  move}  turn_right}"
