...
class KarelStateGenerator(object):
    ...
    # generate an initial env for fourCorners problem
    def generate_single_state_four_corners_random(self, h=12, w=12, wall_prob=0.1, env_task_metadata={}, is_top_off=False):
        """
        initial state generator for four corners problem
        Valid program for four corners problem:
        DEF run m( WHILE c( noMarkersPresent c) w( WHILE c( frontIsClear c) w( move w) IF c( noMarkersPresent c) i( putMarker turnLeft move i) w) m)
        ...
        # randomly select agent position
        candidate_agent_pos = []
        for i in range(1, h-1):
            for j in range(1, w-1):
                candidate_agent_pos.append([i, j])

        agent_pos = random.sample(candidate_agent_pos, 1)[0]
        agent_face = random.randint(0, 3)
        s[agent_pos[0], agent_pos[1], agent_face] = True
    ...
    def generate_single_state_top_off_pick(self, h=12, w=12, wall_prob=0.1, env_task_metadata={}, is_top_off=True):
        ...
        # randomly put markers at row h-2
        marker_num = (self.rng.rand(w-2) > 1 - wall_prob*4).astype(int)
        for m_id in range(9):
            # pdb.set_trace()
    ...
    # generate an initial env for sedder problem
    def generate_single_state_seeder(self, h=8, w=8, wall_prob=0.1, env_task_metadata={}, is_top_off=False):
        ...
        Valid program for seeder problem:
        DEF run m( WHILE c( markersPresent c) w( WHILE c( markersPresent c) w( pickMarker move w) turnRight move turnLeft WHILE c( markersPresent c) w( pickMarker move w) turnLeft move turnRight w) m)
    ...
    # generate an initial env for door key problem
    def generate_single_state_doorkey(self, h=8, w=8, wall_prob=0.1, env_task_metadata={}, is_top_off=False):
        ...
        # randomly place key
        key_pos = random.sample(candidate_agent_pos, 1)[0]
        s[key_pos[0], key_pos[1], 6] = True

        # randomly place target
        candidate_target_pos = []
        for i in range(1, h-2):
            for j in range(5, 7):
                candidate_target_pos.append([i, j])
        target_pos = random.sample(candidate_target_pos, 1)[0]
        s[target_pos[0], target_pos[1], 6] = True
    ...
    def generate_single_state_up_down_stair_climber(self, h=12, w=12, wall_prob=0.1, env_task_metadata={}):
        ...
        while r > 6 and c < w:
            ...
            r -= 1
        r += 2
        valid_agent_pos.append((r - 3, c))
        valid_init_pos.append((r - 3, c))
