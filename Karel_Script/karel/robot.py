...
# single karel robot
class KarelRobot:
    def __init__(self, task, seed=999, prob_mode=False):
        ...
        self.action_steps = 0
        # self.d
        if self.task in ['cleanHouse', 'harvester', 'seeder']:
            self.max_steps = 1000
        elif self.task in ['doorkey']:
            self.max_steps = 250
        else:
            self.max_steps = 100
    ...
    def state_init(self, gen, task):
        ...
        elif task == 'fourCornersRandom':
            gen_function = gen_function_base + "four_corners_random"
