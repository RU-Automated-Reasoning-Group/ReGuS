...
# node in search tree
class SearchNode:
    def __init__(self, parent_node, current_sketch, cost, isleaf=False, score_C=1., cost_ratio=.5):
        # store
    ...
    def self_extend(self, score_C, cost_ratio, add_children=True):
        if not self.isleaf:
...
# MCTS
class SearchTree:
    def __init__(self, root, alg, rollout_limit=10, score_C=1., cost_ratio=.5):
        self.root = root
        self.alg = alg
    ...
    def traverse(self, node=None):
        ...
        while (not node.isleaf and node.visited) or force_true:
            # expand node
            if node.child_num == 0:
                node.self_extend(self.node_score_C, self.node_cost_ratio)
            # find next
