...
class Node:
    ...
    def add_queue(self, candidate, reward, robot, cost=None):
        ...
        eval_result, robot, reward = self.eval_program(self.seed, robot, candidate, check_multiple=True)
        ...
        q_tuple = (
            -reward + cost * self.structural_weight,
            time.time(),
            candidate,
            robot,
            (eval_result, reward),
    ...
    def eval_program(self, seed, robot, candidate, check_multiple):
        ...
        for seed, robot_no_fuel, reward, complete in zip(all_seeds, all_no_fuel, all_rewards, all_completes):
            if reward == -1:
                ...
                return self.FAIL_TYPE, eval_robot, r
        ...
        if np.mean(np.array(all_rewards) >= cur_goal) == 1:
            ...
            if self.cur_goal_idx == len(self.sub_goals) - 1:
                ...
                return self.SUCCESS_TYPE, eval_robot, cur_goal
                ...
                if valid:
                    ...
                    return self.NEXT_GOAL_TYPE, eval_robot, np.mean(all_rewards)
                else:
    ...
    def search(self):
        ...
        while True:
            ...
            for iter in range(self.max_search_iter):
                ...
                # Execute Program
                if state is None:
                    eval_result, eval_robot, eval_reward = self.eval_program(self.seed, robot, p, check_multiple=False)
                else:
                ...
                for i in index:
                    ...
                    # attempt to drop C for while finish
                        ...
                        if end_pos != w_stmt.start_pos and new_robot.execute_single_cond(c_cond):
                            c_stmts, c_idx = candidate.find_actions(c_touch=True)
                            c_stmts.pop(c_idx)
        ...
        return all_rewards, self.timesteps
