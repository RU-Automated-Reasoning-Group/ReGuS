...
class Program:
    ...
    def execute(self, robot, stop=False, drl=True):
        # for debug
        # global state_store
        ...
        if drl:
            return robot.o_list, robot.a_list, robot.a_label_list, robot.r_list, robot.check_reward()==-1
    ...
    def _find(self, stmts):
        ...
        for idx, code in enumerate(stmts):
            ...
            elif isinstance(code, (ACTION, h_cond, C, END)):
                pass
            else:
                raise ValueError('Invalid code')
    ...
    def _find_break_point(self, stmts):
        r_stmts, r_idx = None, None
        for idx, code in enumerate(stmts):
            if isinstance(code, (S, B, C, HIDE_ACTION, h_cond, END)):
                pass
            elif isinstance(code, (WHILE, IF, IFELSE)):
    ...
    def expand_actions(self, c_touch=False, while_drop=True):
        p_list = []
        action_list = []
        new_prog = copy.deepcopy(self)
