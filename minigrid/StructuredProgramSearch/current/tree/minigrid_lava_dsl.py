...
COND_DICT = {
    ...
    "front_is_lava": k_cond(negation=False, cond=k_cond_without_not("front_is_lava")),
    ...
    "not(front_is_lava)": k_cond(negation=True, cond=k_cond_without_not("front_is_lava")),
}
...
class ACTION:
    ...
    def execute_and_update(self, robot):
        if robot.active:
            if self.abs_state is None:
                self.abs_state = get_abs_state(robot)
                self.abs_state.update("front_is_lava", "F")
            new_abs_state = get_abs_state(robot)
            new_abs_state.update("front_is_lava", "F")
            ...
            if self.post_abs_state is None:
                ...
                self.post_abs_state.update("front_is_lava", "DNC")
            new_post_state = get_abs_state(robot)
            new_post_state.update("front_is_lava", "DNC")
...
def get_abs_state_from_list(lst):
    ...
    state.state["front_is_lava"] = lst[6]
