...
# used for push env
ACTION_DICT = {
    "move_to_push_position": k_action("move_to_push_position"),
...
COND_DICT = {
    ...
    "gripper_above_push_position": k_cond(
        negation=False, cond=k_cond_without_not("gripper_above_push_position")
    ),
    "good_to_push": k_cond(negation=False, cond=k_cond_without_not("good_to_push")),
    "not(block_at_goal)": k_cond(
        negation=True, cond=k_cond_without_not("block_at_goal")
    ),
    "not(gripper_above_push_position)": k_cond(
        negation=True, cond=k_cond_without_not("gripper_above_push_position")
    ),
    "not(good_to_push)": k_cond(negation=True, cond=k_cond_without_not("good_to_push")),
}
...
class ABS_STATE:
    def __init__(self):
        self.state = {
            "block_at_goal": None,
            "gripper_above_push_position": None,
            "good_to_push": None,
        }
...
def get_abs_state_from_list(lst):
    ...
    state.state["gripper_above_push_position"] = lst[1]
    state.state["good_to_push"] = lst[2]
