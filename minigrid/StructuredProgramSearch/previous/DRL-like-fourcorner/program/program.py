...
class SimpleProgram:
    def __init__(self):
        self.cond_outer = k_cond(negation=False, cond=k_cond_without_not('no_markers_present'))
        self.cond_inner = k_cond(negation=False, cond=k_cond_without_not('front_is_clear'))
        self.stmts_inner = []
        self.stmts_outer = []
    ...
    def print(self, prefix=''):
        ...
        p_str = prefix + 'DEF run (m WHILE (no_markers_present) (w '
        
        p_str += 'WHILE (not front_is_clear) (w '
        for s in self.stmts_inner:
            p_str += str(s)
            p_str += ' '
        p_str += 'w) '
