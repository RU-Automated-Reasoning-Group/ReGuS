...
class k_action:
    ...
    def __call__(self, k, robot=None):
        ...
        if not robot.no_fuel():
            # TODO: record effective steps
            #prev_pos = robot.checker.get_hero_pos(robot.get_state())
            getattr(k, self.action)()
            #current_pos = robot.checker.get_hero_pos(robot.get_state())
            
            #if current_pos != prev_pos:
            #    robot.effective_steps += 1
            #    robot.moved = True
            #else:
            #    robot.moved = False
