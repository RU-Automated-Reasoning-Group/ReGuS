...
class KarelStateGenerator(object):
    ...
    # TODO: currently, not used for print integer array
    ...
    def generate_single_state_top_off(self, h=12, w=12, wall_prob=0.1, env_task_metadata={}, is_top_off=True):
        ...
        # TODO: tmp code, make sure init pos no marker & at least one marker in the last row
        # markers_map = self.rng.rand(w-2) > 1 - wall_prob
        # markers_map[0] = False
        # markers_map[int(self.rng.rand(1) * (w - 3)) + 1] = True
        # s[h-2, 1:w-1, 6] = markers_map
        # print(markers_map)
        # self.print_state(s)

        # TODO: keep it simple
        markers_map = self.rng.rand(w-2) > 1 - wall_prob
        #markers_map[0], markers_map[5] = False, True
        #markers_map[0] = False
        markers_map[0], markers_map[random.choice(list(range(1, w-2)))] = False, True
        s[h-2, 1:w-1, 6] = markers_map
