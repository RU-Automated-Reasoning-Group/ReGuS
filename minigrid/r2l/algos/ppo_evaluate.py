...
class PPO_Worker:
  ...
  def evaluate(self, trajs=1, max_traj_len=100, store=False):
    # debug
    if store:
      action_store = []
      reward_store = []
    ...
    with torch.no_grad():
      ...
      for traj in range(trajs):
        ...
        while not done and traj_len < max_traj_len:
          ...
          global file_id
          # cv2.imwrite(f"img{file_id:06d}.png", data)
          file_id += 1
    ...
    if store:
      return np.mean(ep_returns), action_store, reward_store
...
def run_experiment(args):
  ...
  if 'karel' in args.env or 'highway' in args.env:
  ...
  # all_rewards = []
  # all_values = []
  # while len(all_rewards) < args.eval_num:
  #   eval_reward = [ray.get([w.evaluate.remote(trajs=1, max_traj_len=args.traj_len, store=True) for w in algo.workers])]
  #   all_rewards += [max(result[2]) for result in eval_reward[0]]
  #   all_values += [result[0] for result in eval_reward[0]]
