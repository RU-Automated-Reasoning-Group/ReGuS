...
def env_factory(path, verbose=False, **kwargs):
    ...
    if "digit" in path.lower():
        ...
        if (
            "random_dynamics" in path
            or "dynamics_random" in path
            or "randomdynamics" in path
            or "dynamicsrandom" in path
        ):
    ...
    if "karel-seeder" in path.lower():
        from karel.karel_env import KarelEnv

        return partial(KarelEnv, task="seeder")

    if "karel-doorkey" in path.lower():
        from karel.karel_env import KarelEnv

        return partial(KarelEnv, task="doorkey")

    if "highway" in path.lower():
        from highway_general.highway_gym import HighwayEnv
    ...
    if "MiniGrid" in path:
        ...
        cls = gymnasium.envs.registration.load_env_creator(spec.entry_point)
        ...
        while total_t < min_timesteps:
            ...
            while not done and timesteps < max_traj_len:
                ...
                if hasattr(policy, "get_quantized_states"):
                    ...
                    mem_states[mem] = True
                    print(
                        policy.get_quantized_states(), len(obs_states), len(mem_states)
                    )
        ...
        try:
            ...
            while render_state:
                if isData():
                    ...
                    if c == "w":
                        env.speed = np.clip(
                            env.speed + 0.1, env.min_speed, env.max_speed
                        )
                    if c == "s":
                        env.speed = np.clip(
                            env.speed - 0.1, env.min_speed, env.max_speed
                    ...
                    if c == "r":
                        ...
                        env.period_shift = [0, 0.5]
                    if c == "t":
                        env.phase_add = np.clip(
                            env.phase_add + 1,
                            int(env.simrate * env.min_step_freq),
                            int(env.simrate * env.max_step_freq),
                        )
                    if c == "g":
                        env.phase_add = np.clip(
                            env.phase_add - 1,
                    ...
                    if c == "y":
                        env.height = np.clip(
                            env.height + 0.01, env.min_height, env.max_height
                        )
                    if c == "h":
                        env.height = np.clip(
                            env.height - 0.01, env.min_height, env.max_height
                    ...
                    if c == "o":
                        ...
                        env.ratio[0] = np.clip(
                            env.ratio[0] + 0.01, 0, env.max_swing_ratio
                        )
                    ...
                    if c == "p":
                        ...
                        env.period_shift[0] = np.clip(
                            env.period_shift[0] + 0.05, 0, 0.5
                        )
                    if c == ";":
                        env.period_shift[0] = np.clip(
                            env.period_shift[0] - 0.05, 0, 0.5
                ...
                if not env.vis.ispaused():
                    ...
                    print(
                        "Mirror: {} | Des. Spd. {:5.2f} | Speed {:5.1f} | Sidespeed {:4.2f} | Heading {:5.2f} | Freq. {:3d} | Coeff {},{} | Ratio {:3.2f},{:3.2f} | RShift {:3.2f},{:3.2f} | Height {:3.2f} \ {:20s}".format(
                            mirror,
                            env.speed,
                            actual_speed,
                            env.side_speed,
            ...
            while not done and timesteps < max_traj_len:
                ...
                if noise is None:
                    action = policy.forward(
                        state, update_norm=True, deterministic=False
                    ).numpy()
                else:
                    action = policy.forward(
                        state, update_norm=True
                    ).numpy() + np.random.normal(0, noise, size=policy.action_dim)
                step_returns = env.step(action)
                if len(step_returns) == 4:
                    state, _, done, _ = step_returns
                elif len(step_returns) == 5:
                    state, _, terminated, truncated, _ = step_returns
