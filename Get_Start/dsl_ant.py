...
class ant_action:
    ...
    def __call__(self, env):
        if self.action in ('move'):
            success, done = getattr(env, self.action)()
            return 1 if success else 0
        else:
            getattr(env, self.action)()
            return 0
...
ACTION_DICT = {
    'move'        : ant_action('move'),
    'turn_right'  : ant_action('turn_right'),
    'turn_left'   : ant_action('turn_left'),
}
...
COND_DICT = {
    'front_is_clear'    : ant_cond(negation=False, cond=ant_cond_without_not('front_is_clear')),
    'left_is_clear'     : ant_cond(negation=False, cond=ant_cond_without_not('left_is_clear')),
    'right_is_clear'    : ant_cond(negation=False, cond=ant_cond_without_not('right_is_clear')),
    'present_goal'      : ant_cond(negation=False, cond=ant_cond_without_not('present_goal')),
    'not(front_is_clear)'    : ant_cond(negation=True, cond=ant_cond_without_not('front_is_clear')),
    'not(left_is_clear)'     : ant_cond(negation=True, cond=ant_cond_without_not('left_is_clear')),
    'not(right_is_clear)'    : ant_cond(negation=True, cond=ant_cond_without_not('right_is_clear')),
    'not(present_goal)'      : ant_cond(negation=True, cond=ant_cond_without_not('present_goal')),
}
...
class ACTION:
    ...
    def execute(self, robot, stop):
        ...
        if robot.active and not robot.no_fuel():
            if robot.force_execution:  # without considering abs_state
                ...
                # # debug
                # if 'move' in str(self.action):
                #     if self.post_abs_state is None:
                #         log_and_print('None')
                #     else:
                #         log_and_print(self.post_abs_state.state)
                ...
                if satisfy_abs_state(get_abs_state(robot), self.abs_state):
                    ...
                    # debug
                    # if 'move' in str(self.action):
                    #     log_and_print(new_robot_state.state)
                    ...
                    # self.post_abs_state.state['left_is_clear'] = 'DNC'
    ...
    def get_conflict(self):
        if self.action.action == 'turn_right':
            ...
            return 'turn_right'
        elif self.action.action == 'pick_marker':
            return 'put_marker'
        elif self.action.action == 'put_marker':
            return 'pick_marker'
        else:
            return None
...
class WHILE:
    ...
    def execute(self, robot, stop):
        ...
        if robot.active and not robot.no_fuel():
            ...
            if not self.resume_point:
                # self.start_pos = robot.karel.get_location()[:2].tolist()
                self.start_pos = robot.xy.tolist()
            # do while
            while robot.active and (self.resume_point or (not robot.no_fuel() and robot.execute_single_cond(self.cond[0]))):
                ...
                # state_store.append(robot.karel.s.copy())

                # debug test
                # robot.steps += 1
            
            # check robot position
            # end_pos = robot.karel.get_location()[:2].tolist()
            end_pos = robot.xy.tolist()
            # update
            if not self.resume_point:
...
class Program:
    def __init__(self, set_unit=False):
        self.stmts = [S(), END()]
        self.set_unit = set_unit
        ...
        for idx, code in enumerate(stmts):
            if isinstance(code, (S, B, SN)):
                ...
                if isinstance(code, IFELSE):
                    ...
                    if not r_stmts is None:
                        return r_stmts, r_idx
            elif isinstance(code, (ACTION, ant_cond, C, END)):
                pass
            else:
                raise ValueError('Invalid code')
    ...
    def _find_break_point(self, stmts):
        r_stmts, r_idx = None, None
        for idx, code in enumerate(stmts):
            if isinstance(code, (S, B, C, HIDE_ACTION, ant_cond, END)):
                pass
            elif isinstance(code, (WHILE, IF, IFELSE)):
    ...
    # NOTE: expand sketch
    def expand(self, get_cond=False):
        p_list = []
        cond_list = None
        if get_cond:
            cond_list = []
        ...
        # expand S
            ...
            if self_loop:
                stmts[idx] = C()
                p_list.append(copy.deepcopy(new_prog))
            # S -> C'; while (if set unit)
            if self.set_unit:
                stmts[idx] = C(unit=True)
                stmts.insert(idx+1, WHILE())
                stmts.insert(idx+2, S())
            # S -> while (if not set unit)
            else:
                stmts[idx] = WHILE()
    ...
    def expand_actions(self, c_touch=False, while_drop=True, keep_touch=False):
        ...
        # get conflict action and reject
        conflict_act = None
        if idx > 0 and isinstance(stmts[idx-1], ACTION):
            conflict_act = stmts[idx-1].get_conflict()

        stmts, idx = new_prog.find_actions(c_touch)
        if not stmts is None:
            for action in WRAPPED_ACTION_LIST:
                if action.action.action == conflict_act:
                    continue
            ...
            if found_C.unit or (while_drop and idx>0 and isinstance(stmts[idx-1], WHILE)):
                stmts.pop(idx)
                p_list.append(copy.deepcopy(new_prog))
                action_list.append(None)
                stmts.insert(idx, C(unit=found_C.unit))
        else:
            pass
