...
from programskill.hook_dsl import k_cond, k_cond_without_not, k_action

import pdb

# actions:
# 1. move to the push position (used for move down)
# 2. move to above the push position
# 3. move to goal
# 4. idle
# used for push env
ACTION_DICT = {
    "pick_up_hook": k_action("pick_up_hook"),
    "align": k_action("align"),
    "sweep": k_action("sweep"),
    "idle": k_action("idle"),
}
...
# actions used by libraries
LIBRARY_ACTION_DICT = {
    "move_to_object": k_action("move_to_object"),
    "open_gripper": k_action("open_gripper"),
    "close_gripper": k_action("close_gripper"),
    "move_down": k_action("move_down"),
    "move_to_target": k_action("move_to_target"),
}

# predicate used by libraries
LIBRARY_PREDICATE_DICT = {
    "object_at_target",
    "object_below_gripper",
    "gripper_are_open",
...
# predicates
# 1. block at goal
# 2. gripper above push position
# 3. good to push
# use for push env
COND_DICT = {
    "block_at_goal": k_cond(negation=False, cond=k_cond_without_not("block_at_goal")),
    "hook_grasped": k_cond(negation=False, cond=k_cond_without_not("hook_grasped")),
    "hook_aligned": k_cond(negation=False, cond=k_cond_without_not("hook_aligned")),
    "not(block_at_goal)": k_cond(
        negation=True, cond=k_cond_without_not("block_at_goal")
    ),
    "not(hook_grasped)": k_cond(negation=True, cond=k_cond_without_not("hook_grasped")),
    "not(hook_aligned)": k_cond(negation=True, cond=k_cond_without_not("hook_aligned")),
}
...
class ACTION:
    ...
    def execute(self, robot, stop):
        if robot.active:
            if str(self.action.action) == "pick_up_hook":
            ...
            if robot.force_execution:  # without considering abs_state
                ...
                if satisfy_abs_state(get_abs_state(robot), self.abs_state):
                    r = robot.execute_single_action(self.action)
                    # modify post abstract state here (TODO: whether good to put here?)
                    new_robot_state = get_abs_state(robot)
...
def get_abs_state_from_list(lst):
    ...
    state.state["hook_grasped"] = lst[1]
    state.state["hook_aligned"] = lst[2]
...
def pick_and_place():
    fn = Program()
    while_code = WHILE()
    while_code.cond[0] = get_cond(True, "object_at_target")

    while_code.stmts[0] = IFELSE(get_cond(False, "object_below_gripper"))
    branch1 = while_code.stmts[0]
    branch1.else_stmts[0] = get_action("move_to_object")
    branch1.stmts[0] = IFELSE(get_cond(False, "gripper_are_open"))

    branch2 = branch1.stmts[0]
    branch2.stmts[0] = IFELSE(get_cond(False, "object_inside_gripper"))
    branch3 = branch2.stmts[0]
    branch2.else_stmts[0] = IFELSE(get_cond(False, "object_is_grasped"))
    branch4 = branch2.else_stmts[0]

    branch3.stmts[0] = get_action("close_gripper")
    branch3.else_stmts[0] = get_action("move_down")

    branch4.stmts[0] = get_action("move_to_target")
    branch4.else_stmts[0] = get_action("open_gripper")
...
def hook_ground_truth():
    fn = Program()
    while_code = WHILE()
    while_code.cond[0] = get_cond(True, "block_at_goal")

    while_code.stmts[0] = IFELSE(get_cond(False, "hook_grasped"))
    branch1 = while_code.stmts[0]
    branch1.else_stmts[0] = get_action("pick_up_hook")
    branch1.stmts[0] = IFELSE(get_cond(False, "hook_aligned"))

    branch2 = branch1.stmts[0]
    branch2.stmts[0] = get_action("sweep")
    branch2.else_stmts[0] = get_action("align")
