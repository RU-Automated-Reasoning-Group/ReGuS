...
def state_is_stuck(obs):
    state = obs['observation']
    state = state.reshape(1, -1)
    return model.predict(state)

def get_pick_and_place_control(obs, relative_grasp_position=(0., 0., -0.02), workspace_height=0.1, atol=1e-3, gain=10, block_idx=0):
    """
    Returns
    -------
    action : [float] * 4
    ...
    if np.sum(np.subtract(block_position, place_position)**2) < atol:
        ...
        return np.array([0., 0., 0.01, 1.]), True
    ...
    if block_is_grasped(obs, relative_grasp_position, block_position, atol=atol):
        ...
        # If current state is stuck
        if state_is_stuck(obs):
            # Move up the gripper while keep closed
            if DEBUG:
                print("Move up the gripper")
            return np.array([0., 0., 5, -1.]), False
