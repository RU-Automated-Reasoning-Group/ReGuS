...
class FetchPush(gym.Env):
    ...
    def block_at_goal(self, atol=1e-4):
        ...
        return np.linalg.norm(np.subtract(block_position, place_position)) < 0.03

    def gripper_above_push_position(self, atol=1e-4):
        gripper_position, _, _ = self.get_obs()
        push_position, _, _, _ = self.compute_push_position_stat()
        return (gripper_position[0] - push_position[0]) ** 2 + (
            gripper_position[1] - push_position[1]
        ) ** 2 < atol

    def good_to_push(self, atol=1e-4):
        gripper_position, _, _ = self.get_obs()
        (
            push_position,
            angle_diff,
            block_sq_distance,
            gripper_sq_distance,
        ) = self.compute_push_position_stat()
        return (
            (gripper_position[2] - push_position[2]) ** 2 < atol
            and angle_diff < np.pi / 4
            and block_sq_distance < gripper_sq_distance
        )

    def compute_push_position_stat(self, block_width=0.15):
        gripper_position, block_position, goal = self.get_obs()

        desired_block_angle = np.arctan2(
            goal[0] - block_position[0], goal[1] - block_position[1]
        )
        gripper_angle = np.arctan2(
            goal[0] - gripper_position[0], goal[1] - gripper_position[1]
        )

        push_position = block_position.copy()
        push_position[0] += -1.0 * np.sin(desired_block_angle) * block_width / 2.0
        push_position[1] += -1.0 * np.cos(desired_block_angle) * block_width / 2.0
        push_position[2] += 0.005

        angle_diff = abs(
            (desired_block_angle - gripper_angle + np.pi) % (2 * np.pi) - np.pi
        )

        gripper_sq_distance = (gripper_position[0] - goal[0]) ** 2 + (
            gripper_position[1] - goal[1]
        ) ** 2
        block_sq_distance = (block_position[0] - goal[0]) ** 2 + (
            block_position[1] - goal[1]
        ) ** 2

        return push_position, angle_diff, block_sq_distance, gripper_sq_distance
