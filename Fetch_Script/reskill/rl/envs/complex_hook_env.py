...
class ComplexHookSingleObjectEnv(fetch_env.MujocoPyFetchEnv, utils.EzPickle):
    def __init__(self, xml_file=None, render_mode = None):
        ...
        fetch_env.MujocoPyFetchEnv.__init__(
            self, model_path=xml_file, has_object=True, block_gripper=False, n_substeps=20,
...
class ComplexHookEnv(gym.Env):
    def __init__(self, train=True, num_scenes=100, scene_offset=0, subenv_type='normal', render_mode = None):
        if train:
        ...
        for xml_file in xml_files:
            try:
                if subenv_type == 'residual':
                    ...
                    subenv = ComplexHookSingleObjectEnv(xml_file, render_mode=render_mode)
                else:
                    print('subenv_type not recognized')
                self._subenvs.append(subenv)
            except Exception as error:
                print(f"FAILED; skipping with error {error}")
    ...
    def reset(self):
        env_id = self.np_random.integers(self._num_envs)
    ...
    def step(self, action):
        ...
        try:
            obs, reward, done1, done2, info = self._env.step(action)
        except:
            print('Action: ', action)
            obs, reward, done1, done2, info = self.back_up_obs, -1.0 , False, False, {'is_success': 0.0}
