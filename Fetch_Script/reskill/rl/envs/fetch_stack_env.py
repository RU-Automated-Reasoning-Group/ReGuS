...
class FetchStackEnv(robot_env.RobotEnv):
    ...
    def compute_reward(
        ...
        if self.reward_info == "incremental":
            ...
            # np.putmask(reward, reward == -1, 0.0)
            print("a")
            return reward
            ...
            dist_b = np.linalg.norm(achieved_goal[0:3] - goal[0:3], axis=-1)
            ...
            if (
                achieved_goal[3] > 1.30
                and achieved_goal[3] < 1.5
                and achieved_goal[4] > 0.96
                and achieved_goal[4] < 1.16
            ):
            ...
            if (
                achieved_goal[0] > 1.30
                and achieved_goal[0] < 1.5
                and achieved_goal[1] > 0.96
                and achieved_goal[1] < 1.16
            ):
                if force_sensor > 19625:
                    rew_r = 1.0
                else:
    ...
    def _get_obs(self):
        ...
        if self.use_force_sensor:
            ...
            )  # Read force sensor reading from tray
        else:
            force_reading = [0, 0, 0]
    ...
    def reset(self):
        ...
        for i, obj_name in enumerate(self.object_names):
            ...
            print(f"initial box position is {object_qpos[:2]}")
    ...
    def _sample_goal(self, return_extra_info=False):
        ...
        if self.np_random.uniform() < 0.5:
            print("move goal0 to air")
            goal0[2] += self.np_random.uniform(0, 0.35)
        ...
        for i in range(self.num_blocks - 1):
            if i < number_of_goals_along_stack - 1:
                ...
                if self.np_random.uniform() < 0.5:
                    print("move goal to air")
                    goal_i[2] += self.np_random.uniform(0, 0.35)
